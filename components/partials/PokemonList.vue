<template>
  <div class="black pok-fill--all overflow-auto">
    <v-text-field hide-details append-icon="fas fa-search" class="pok-input-text-search pa-0 ma-0" @input="search" />
    <div>
      <v-list two-line class="transparent pa-0">
        <v-list-tile v-for="pokemon in pokemonList" :key="pokemon.id" class="pok-third-brand pb-3 bt-2 mb-1">
          <v-list-tile-avatar>
            <img :src="pokemon.sprite" class="pok-img--sz3 mt-3" />
          </v-list-tile-avatar>
          <v-list-tile-content>
          ...
          <v-list-tile-avatar>
            <button class="white--text pok-text--h2 mt-3" @click="choosedPokemon(pokemon.id)"><i class="fas fa-plus" /></button>
          </v-list-tile-avatar>
        </v-list-tile>
      </v-list>
    </div>
